...
- 模拟二极管、电容、电流表和电压表，并给出第一版教学反馈
- 使用节点/支路模型求解简单电路，驱动支路电流、等效电阻和电表读数
- 在通电导线上显示电流流动动画
...
- 在小屏/PWA 布局中将 GitHub、复位、清线、导出和缩放统一收进底部 HUD
- 在小屏/PWA 布局中使用更接近全屏画布的工作台，而不是带边框的白板
...
- 根据当前工作台生成并导出实体搭建清单，包含物料数量、采购关键词、接线步骤和可打印装配单
- 根据当前实验生成可复制/导出的 Markdown 实验报告，包含任务进度、回路结论、元器件、接线记录和观察栏
- 显示回路状态、电流、等效电阻和灯泡亮度
...
- 开关可以断开或闭合电路
- 导线和闭合开关会把端子合并为电气节点
- 电阻、灯泡、蜂鸣器、电机、电流表，以及正向 LED/二极管会作为支路参与求解
- 支路电流会驱动灯泡亮度、蜂鸣器强度、电机速度和电流表读数
- 电压表和电容会读取两端节点电压，但不会替代导线闭合回路
- LED 和二极管只有在正向连接时才会导通
...
13. 使用 **实体** 查看、复制或导出当前电路的物料清单、接线步骤和可打印装配单。
14. 使用 **清线** 断开所有导线。
15. 使用 **复位** 恢复默认演示电路。
...
- v0.4 第一版：蜂鸣器、电机、二极管、电容、电流表和电压表元器件
- v0.4 第一版：节点/支路仿真、支路电流、等效电阻和电表读数
- v0.4 第一版：仿真内核和实体搭建清单回归测试
- 产品方向第一版：根据当前工作台生成可复制/导出的实体物料清单、采购关键词、接线步骤、可打印装配单和实验报告
