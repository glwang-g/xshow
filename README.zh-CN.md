...
`xshow circuits` 是一个早期教育原型，目标是让用户像拼电子积木一样搭建简单电路。你可以把电池、开关、灯泡、蜂鸣器、导线、可变电阻器连接起来，切换开关，调节电阻，并立即看到电路状态变化。
...
7. 在手机和平板上，可以用底部 HUD 呼出器件面板、状态浮窗、导出图片、回正视图、缩放工作台。
8. 使用 **导出** 将当前工作台保存为 PNG 图片。
9. 使用 **记录** 暂存和加载带名称的本地工作台快照。
10. 使用 **导入 JSON / 导出 JSON** 在不同设备之间迁移当前工作台。
11. 使用 **复制分享链接** 把当前工作台状态发送给别人。
12. 使用 **清线** 断开所有导线。
13. 使用 **复位** 恢复默认演示电路。
