...
The first implementation adds lesson entries for Relay, NOT, AND, and OR, plus normally-open/normally-closed contacts. The Logic Lab now composes published, truth-table-verified AND, OR, and NOT modules into a half adder: `SUM = (A OR B) AND NOT(A AND B)` and `CARRY = A AND B`, then combines two half adders and OR into a full adder with `Cin/Cout`; every participating module remains expandable back in the workshop. The algorithm layer reuses that full adder for eight-bit ripple-carry addition and shows each `A + B + Cin → SUM / Cout` step, so a learner can compare the current program input directly with the gate-level prediction. XOR, half adders, and full adders as independently publishable composition modules/netlists remain the next increment.
