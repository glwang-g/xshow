# Simulation Contract 迁移评估

[English version](simulation-contract.md)

本文记录第一次迁移评估：如何把 `xshow` 从多个相对独立的 Lab，逐步收束为统一的模拟契约。`World` 暂时保留为未来更丰富交互场景的产品概念，不作为当前运行时名称。

目标不是重写。第一步只建立共享模拟契约：`Simulation`、`Entity`、`Component`、`State`、`Tick`、`Action`、`Rule`、`Scenario`、`Replay`、`Visualization`。

## 现有抽象映射

| Simulation 抽象 | 当前对应 | 备注 |
| --- | --- | --- |
| `Simulation` | `TankLabBattle`、`ComputerCoreApi`、电路工作台状态、Logic Lab 预览状态 | Tank Lab 最接近完整的交互模拟。Computer Lab 已经有 core API 边界。Circuit 与 Logic 目前更像规则/求值模块。 |
...
| 模式 | 当前重复点 | 迁移方向 |
| --- | --- | --- |
| run/step/reset 循环 | `ComputerLab.vue` 包装 core action；`TankLab.vue` 自己维护运行/暂停/单步/重置和动画循环；Logic Lab 有 apply/reset | 至少两个 adapter 接入 `Simulation.tick()` 后，再抽共享 simulation-controller/composable。 |
| Scenario 数据 | Tank preset 在 `TankLab.vue`；CPU sample program 在 `computer-core.ts`；Logic default trace 在 `logic-core.ts`；电路 lesson 在 lesson data | 未来新增 scenario 统一走 `SimulationScenario`。既有内容逐步迁移。 |
| Replay/trace/event | CPU `log`、Tank `events`、Logic trace arrays、workspace records | 先定义 `SimulationReplay` 类型。之后从 adapter 记录 frame。 |
...
| 状态与 tick 文案 | `actionMessage`、`battleStatus`、`resultInsight`、电路状态面板、lesson hints | 以后从 simulation state/events 暴露状态，但第一阶段保留现有 UI 文案。 |

## 最小迁移路线

1. **只建契约：** 新增 `src/lib/simulation-contract.ts`，只放 TypeScript 接口，不接入运行时。
2. **沉淀边界：** 保留本文和 `docs/layers.md` 作为迁移地图。
3. **逐个薄适配：**
   - `CircuitSimulation`：输入 parts/wires；rule 调用 `evaluateCircuit()`。
   - `LogicSimulation`：action 切换输入或脉冲存储；rule 调用 `evaluateGate()` 和 `srLatchStep()`。
   - `ComputerSimulation`：包装 `ComputerCoreApi`；先 preview，后 WASM。
   - `TankBattleSimulation`：包装 `stepTankBattle()` 并记录 replay frame。
4. **等 adapter 成型后再抽 controller：** `useSimulationRunner()` 可以统一 run/pause/step/reset/replay capture。
...
- 新增纯类型文件 `src/lib/simulation-contract.ts`。
...
第一版类型已放在 `src/lib/simulation-contract.ts`：
