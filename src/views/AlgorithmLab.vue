...
import { Braces, CircuitBoard, Cpu, Home, Play, RotateCcw, StepForward } from "@lucide/vue";
import { RouterLink } from "vue-router";
import logoUrl from "@/assets/logo.png";
import { blankCpuSnapshot, createComputerCore, formatHex, type ComputerCoreApi } from "@/lib/computer-core";
import { composeByteAdder } from "@/lib/logic-composition";
...
const lastRun = ref<{ left: number; right: number; result: number; carry: boolean } | null>(null);
...
const byteAdder = computed(() => composeByteAdder(publishedModules.value, normalizedLeft.value, normalizedRight.value));
const hasCurrentRun = computed(() => lastRun.value?.left === normalizedLeft.value && lastRun.value?.right === normalizedRight.value);
const gateMatchesMachine = computed(() => Boolean(hasCurrentRun.value && byteAdder.value.available && lastRun.value
  && byteAdder.value.sum === lastRun.value.result && byteAdder.value.carry === lastRun.value.carry));

function normalizeInputs() {
  left.value = normalizedLeft.value;
  right.value = normalizedRight.value;
}
...
async function loadAndRun() {
  ...
  try {
    ...
    lastRun.value = {
      ...
      result: snapshot.value.memory[0x40] ?? 0,
  ...
  <main class="min-h-screen bg-[linear-gradient(180deg,#f8fbfa_0%,#eef5f3_100%)] text-slate-900">
    <div class="mx-auto flex min-h-screen w-full max-w-[1120px] flex-col gap-5 px-4 py-4 lg:px-6">
      ...
      <section class="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div class="flex items-center gap-2 text-sm font-semibold"><Braces class="h-4 w-4 text-cyan-700" />输入数据</div><label class="mt-4 block text-xs font-medium text-slate-500">第一个数 A<input v-model.number="left" type="number" min="0" max="255" class="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm text-slate-900" @blur="normalizeInputs" /></label><label class="mt-3 block text-xs font-medium text-slate-500">第二个数 B<input v-model.number="right" type="number" min="0" max="255" class="mt-1 h-10 w-full rounded-md border border-slate-200 px-3 text-sm text-slate-900" @blur="normalizeInputs" /></label><p class="mt-2 text-xs leading-5 text-slate-500">输入会规范为 0–255，再写入 8-bit 寄存器。</p><div class="mt-5 grid grid-cols-2 gap-2"><button type="button" :disabled="isBusy" class="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-slate-950 px-3 text-sm font-medium text-white disabled:bg-slate-300" @click="loadAndRun"><Play class="h-4 w-4" />运行</button><button type="button" :disabled="isBusy" class="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-slate-200 px-3 text-sm font-medium text-slate-700 disabled:text-slate-300" @click="loadAndStep"><StepForward class="h-4 w-4" />单步</button></div><button type="button" :disabled="isBusy" class="mt-2 inline-flex h-9 w-full items-center justify-center gap-2 rounded-md text-xs font-medium text-slate-600 hover:bg-slate-50" @click="resetMachine"><RotateCcw class="h-3.5 w-3.5" />复位机器</button></div>
        <div class="min-w-0 space-y-4"><section class="rounded-xl border border-slate-200 bg-slate-950 p-5 text-slate-100 shadow-sm"><div class="flex items-center justify-between gap-3"><div class="text-sm font-semibold">程序</div><span class="rounded bg-white/10 px-2 py-1 text-xs">A + B → 0x40</span></div><pre class="mt-4 overflow-x-auto font-mono text-sm leading-6 text-cyan-100">{{ source }}</pre></section><section class="grid gap-3 sm:grid-cols-3"><div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div class="text-xs text-slate-500">寄存器 A</div><div class="mt-2 font-mono text-2xl font-semibold">{{ snapshot.a }}</div></div><div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div class="text-xs text-slate-500">寄存器 B</div><div class="mt-2 font-mono text-2xl font-semibold">{{ snapshot.b }}</div></div><div class="rounded-xl border border-cyan-200 bg-cyan-50 p-4 shadow-sm"><div class="text-xs text-cyan-700">内存 0x40</div><div class="mt-2 font-mono text-2xl font-semibold text-cyan-950">{{ result }} <span class="text-sm font-normal">(0x{{ formatHex(result) }})</span></div></div></section><section class="rounded-xl border border-cyan-200 bg-white p-4 text-sm shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2"><div><div class="font-medium text-slate-900">我的门级 8-bit 加法器</div><p class="mt-1 text-xs leading-5 text-slate-500">8 个可追溯的全加器串接，预测本次 A + B 的结果。</p></div><span class="rounded px-2 py-1 font-mono text-xs font-semibold" :class="byteAdder.available ? 'bg-emerald-50 text-emerald-800' : 'bg-amber-50 text-amber-800'">{{ byteAdder.available ? '逻辑基座已就绪' : `缺少 ${byteAdder.missing.join(' / ')}` }}</span></div><template v-if="byteAdder.available"><div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3"><div class="rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-3"><div class="text-xs text-cyan-700">门级 SUM</div><div class="mt-1 font-mono text-2xl font-semibold text-cyan-950">{{ byteAdder.sum }}</div></div><div class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-3"><div class="text-xs text-amber-700">门级 Cout</div><div class="mt-1 font-mono text-2xl font-semibold text-amber-950">{{ Number(byteAdder.carry) }}</div></div><div class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3"><div class="text-xs text-slate-500">CPU 预期</div><div class="mt-1 font-mono text-2xl font-semibold text-slate-950">{{ expectedByteResult }}</div></div></div><p v-if="hasCurrentRun" class="mt-3 rounded-md px-3 py-2 text-xs font-medium" :class="gateMatchesMachine ? 'bg-emerald-50 text-emerald-800' : 'bg-rose-50 text-rose-800'">{{ gateMatchesMachine ? '已验证：门级预测与本次机器运行一致。' : '注意：门级预测与本次机器运行不一致，请检查模块验证和运行状态。' }}</p><div class="mt-3 flex flex-wrap items-center gap-1.5 text-xs"><span class="mr-1 text-slate-500">门来源：</span><RouterLink v-for="module in byteAdder.modules" :key="module.id" :to="{ path: '/workbench/workshop', query: { module: module.id } }" class="rounded border border-cyan-200 bg-cyan-50 px-2 py-1 font-medium text-cyan-800 hover:bg-cyan-100">{{ module.name }}</RouterLink></div><div class="mt-3 flex flex-wrap gap-1.5"><span v-for="step in byteAdder.steps" :key="step.bit" class="rounded border border-slate-200 bg-slate-50 px-2 py-1 font-mono text-[11px] text-slate-600">b{{ step.bit }}: {{ Number(step.a) }}+{{ Number(step.b) }}+{{ Number(step.carryIn) }} → {{ Number(step.sum) }} / C{{ Number(step.carryOut) }}</span></div></template><RouterLink v-else to="/workbench/workshop" class="mt-3 inline-flex h-8 items-center gap-1 rounded border border-cyan-200 bg-cyan-50 px-2 text-xs font-medium text-cyan-800 hover:bg-cyan-100"><CircuitBoard class="h-3.5 w-3.5" />去制作缺少的逻辑门</RouterLink></section><section class="rounded-xl border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-600"><div class="font-medium text-slate-900">机器观察</div><p class="mt-1">{{ message }}</p><p class="mt-2 text-xs text-slate-500">数学结果：{{ normalizedLeft }} + {{ normalizedRight }} = {{ mathematicalSum }} · 8-bit 写入：{{ expectedByteResult }}{{ overflowsByte ? '（超过 255，进位保留在 FLAGS.C）' : '' }}</p><p class="mt-2 text-xs text-slate-500">PC：0x{{ formatHex(snapshot.pc) }} · {{ snapshot.currentInstruction }} · {{ snapshot.halted ? "已停止" : "执行中" }} · Carry：{{ Number(snapshot.flags.c) }}</p></section></div>
      </section>
    </div>
  </main>
</template>
