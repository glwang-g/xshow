...
test("desktop workbench regions stay in one explicit grid row after deferred panels mount", () => {
  ...
  assert.match(homeSource, /<div v-if="desktopViewport \|\| palettePanelOpen" class="contents xl:row-start-1 xl:block xl:min-h-0"/);
  assert.match(homeSource, /<WorkbenchCanvas\s+class="xl:row-start-1"/);
  assert.match(homeSource, /<div v-if="desktopViewport \|\| statusPanelOpen" class="contents xl:row-start-1 xl:block xl:min-h-0"/);
