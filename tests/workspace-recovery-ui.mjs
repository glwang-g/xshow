...
const recordsSource = await readFile(new URL("../src/composables/useWorkspaceRecords.ts", import.meta.url), "utf8");
...
test("invalid share and autosave recovery has a visible records-panel explanation", () => {
  assert.match(recordsSource, /分享链接中的工作台数据无效，已忽略该链接。/);
  assert.match(recordsSource, /上次自动保存的数据不完整，已安全回退到默认工作台。/);
  ...
  assert.match(recordsSource, /options\.loadSnapshot\(parsed\);\s+options\.recoveryMessage\.value = ""/);
});
